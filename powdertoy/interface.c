...
void menu_count(void)//puts the number of elements in each section into .itemcount
{
	...
	msections[SC_LIFE1].itemcount = msections[SC_LIFE2].itemcount = NGOLALT/2;
...
void draw_svf_ui(pixel *vid_buf, int alternate)// all the buttons at the bottom
{
    ...
    fillrect(vid_buf, XRES-17+BARSIZE/*493*/, YRES+(MENUSIZE-17), 16, 16, 255, 255, 255, 255);
    fillrect(vid_buf, XRES-15+BARSIZE/*493*/, YRES+(MENUSIZE-15), 12, 4, 0, 0, 0, 255);
    fillrect(vid_buf, XRES-15+BARSIZE/*493*/, YRES+(MENUSIZE-11), 12, 4, 0, 0, 0, 255);
    fillrect(vid_buf, XRES-15+BARSIZE/*493*/, YRES+(MENUSIZE-7), 12, 4, 0, 0, 0, 255);
    ...
    drawtext(vid_buf, 22, YRES+(MENUSIZE-14), "\x82", 255, 255, 255, 255);
    if(svf_fileopen)
        drawtext(vid_buf, 40, YRES+(MENUSIZE-12), svf_filename, 255, 255, 255, 255);
    else
        drawtext(vid_buf, 40, YRES+(MENUSIZE-12), "[save to disk]", 255, 255, 255, 255);
    drawrect(vid_buf, 19, YRES+(MENUSIZE-16), 150, 14, 255, 255, 255, 255);
    ...
    drawtext(vid_buf, XRES-46+BARSIZE/*139*/, YRES+(MENUSIZE-14), "\x92", 255, 255, 255, 255);
	drawrect(vid_buf, XRES-50+BARSIZE/*143*/, YRES+(MENUSIZE-16), 16, 14, 255, 255, 255, 255);
	...
	{//to kurwa
		drawtext(vid_buf, XRES-63+BARSIZE/*481*/, YRES+(MENUSIZE-13), "\xCF", 255, 255, 255, 255);
		drawrect(vid_buf, XRES-66+BARSIZE/*494*/, YRES+(MENUSIZE-16), 14, 14, 255, 255, 255, 255);
	}
...
void menu_ui_v3(pixel *vid_buf, int i, int *sl, int *su, int mx, int my)
{
	...
	else if(i==SC_LIFE1||i==SC_LIFE2)
	{
		int n2;
		
		//we don't need any scrolling, even fancy...
		/*if (fwidth > XRES-BARSIZE) { //fancy scrolling
			float overflow = fwidth-(XRES-BARSIZE), location = ((float)XRES-BARSIZE)/((float)(mx-(XRES-BARSIZE)));
			xoff = (int)(overflow / location);
		}*/
    ...
    if(i==SC_WALL) {
        switch((*sl)-100) {
            ...
            case WL_ALLOWGAS:
            case WL_ALLOWENERGY:
                drawtext(vid_buf, XRES-textwidth((char *)wtypes[(*sl)-UI_WALLSTART].descs)-8, sy-10, (char *)wtypes[(*sl)-UI_WALLSTART].descs, 255, 255, 255, 255);
                break;
            default:
                drawmenu=1;
        }
        ...
        if ((*sl)==SPC_AIR||(*sl)==SPC_HEAT||(*sl)==SPC_COOL||(*sl)==SPC_VACUUM||(*sl)==SPC_WIND||(*sl)==SPC_PGRV||(*sl)==SPC_NGRV) drawtext(vid_buf, XRES-textwidth((char *)wtypes[(*sl)-UI_WALLSTART].descs)-8, sy-10, (char *)wtypes[(*sl)-UI_WALLSTART].descs, 255, 255, 255, 255);
        else drawmenu=1;
    }
	else if (i==SC_LIFE1||i==SC_LIFE2)
	{
        int selector=((*sl)>>8)&0xFF;
        
		if(i==SC_LIFE2) {
            if(selector>=0&&selector<=11) drawmenu=1;
		}
		else {
			if((selector>=12&&selector<=24)||(selector==0&&(*sl)!=GT_GOL)) drawmenu=1;
		}
        
		if(!drawmenu) {
            drawtext(vid_buf, XRES-textwidth((char *)gmenu[selector].description)-8, sy-10, (char *)gmenu[selector].description, 255, 255, 255, 255);
        }
	}
	else
	{
        if (ptypes[*sl].menusection==i) drawtext(vid_buf, XRES-textwidth((char *)ptypes[*sl].descs)-8, sy-10, (char *)ptypes[*sl].descs, 255, 255, 255, 255);
        else drawmenu=1;
    }
    
    if(drawmenu) {
        drawtext(vid_buf, XRES-textwidth((char *)msections[i].name)-8, sy-10, (char *)msections[i].name, 255, 255, 255, 255);
    }
...
int64_t firstid=0,secondid=0;

char firsttap,secondtap;
float firstx,firsty;
...
void touchevent() {
    if(firsttap&&secondtap) {
        int pincz=sqrt(pow(firstx-secondx,2)+pow(firsty-secondy,2))*100.0f;
        if(startpincz==-1) {
            startpincz=pincz;
            startbs=bsx;
        }
        else {
            int liczba=startbs+(pincz-startpincz);
            liczba=liczba>30?30:liczba<0?0:liczba;
            bsx=bsy=liczba;
        }

        /*float pincz=sqrt(pow(firstx-secondx,2)+pow(firsty-secondy,2));
        if(pincz>0.4f) pincz=0.4f;
        pincz=(pincz-0.1f)*100.0f;
        bsx=bsy=pincz;*/
    }
}
...
int sdl_poll(void)
{
	...
	while (SDL_PollEvent(&event))
	{
		switch (event.type)
		{
            case SDL_FINGERDOWN:
                if(firstid==0) {
                    firstid=event.tfinger.fingerId;
                    firstx=event.tfinger.x;
                    firsty=event.tfinger.y;
                }
                else if(secondid==0) {
                    secondid=event.tfinger.fingerId;
                    secondx=event.tfinger.x;
                    secondy=event.tfinger.y;
                ...
                break;
            case SDL_FINGERUP:
                //if(event.tfinger.fingerId==firstid) firstid=0;
                //if(event.tfinger.fingerId==secondid) secondid=0;
                firstid=firsttap=0;
                secondid=secondtap=0;
                
                startpincz=-1;
                showtime=0;
                break;
            case SDL_FINGERMOTION:
                if(event.tfinger.fingerId==firstid) {
                    firstx=event.tfinger.x;
                    firsty=event.tfinger.y;
                }
                else if(event.tfinger.fingerId==secondid) {
                    secondx=event.tfinger.x;
                    secondy=event.tfinger.y;
                }
                else break;
...
int save_filename_ui(pixel *vid_buf)
{
	...
	while (!sdl_poll())
	{
		...
		if (sdl_key==SDLK_ESCAPE ||
           (b && !bq && (mx < x0 || mx > x0+xsize || my < y0 || my > y0+ysize)))
		{
			if (!ed.focus)
				break;
			ed.focus = 0;
...
void simulation_ui(pixel * vid_buf)
{
	...
	while (!sdl_poll())
	{
        ...
        const char soon[]="(Feature comming soon!)";
		drawtext(vid_buf, x0+8, y0+200, "Large window", 255, 255, 255, 255);
		drawtext(vid_buf, x0+12+textwidth("Large window"), y0+200, soon/*"Double window size for small screens"*/, 255, 255, 255, 180);
		
		drawtext(vid_buf, x0+8, y0+214, "Fullscreen", 255, 255, 255, 255);
		drawtext(vid_buf, x0+12+textwidth("Fullscreen"), y0+214, soon/*"Fill the entire screen"*/, 255, 255, 255, 180);

		drawtext(vid_buf, x0+8, y0+228, "Block frame", 255, 255, 255, 255);
		drawtext(vid_buf, x0+12+textwidth("Block frame"), y0+228, soon/*"Draws a wall frame around screen"*/, 255, 255, 255, 180);
...
Uint8 mouse_get_state(int *x, int *y)
{
	...
	*y = sdl_y/sdl_scale;
    ...
    //Wrapper around multitouch
    //printf("firstx:%f firsty:%f\n",firstx,firsty);
	*x = firstx*480.0f;
	*y = firsty*360.0f;
