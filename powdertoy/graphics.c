...
{
    SDL_UpdateTexture(texture, NULL, src, w * PIXELSIZE);
...
{
    printf("Not implemented: blit using double scale\n");
...
//draws the cursor
void render_cursor(pixel *vid, int rx, int ry)
{
    if(showtime) {
        if (rx>0) {
            int x=480.0f/2.0f;
...
int sdl_open(void)
{
    ...
    int width = XRES+BARSIZE;
    int height = YRES+MENUSIZE;
    
    window = SDL_CreateWindow(NULL, 0, 0, width, height,SDL_WINDOW_BORDERLESS);
    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    texture = SDL_CreateTexture(renderer,SDL_PIXELFORMAT_ARGB8888,SDL_TEXTUREACCESS_STREAMING,width, height);//1024,768);
